...
for cls_pat in ["news", "post", "card", "item", "entry", "list", "article"]:
...
# 找所有含 /news/ 或 /category/ 的链接
links = soup.find_all("a", href=True)
news_links = [l for l in links if re.search(r'/(news|category)/', l["href"])]
p(f"\n=== News-related links: {len(news_links)} ===")
...
# 深度分析第一个 article 内部结构
def walk_article(el, depth=0):
    if depth > 5:
        return
    for child in el.children:
        ...
        if tag in ("div", "header", "footer", "section"):
            walk_article(child, depth + 1)

if articles:
    p("\n=== First article detailed ===")
    walk_article(articles[0])
...
with open("_mddic_analysis.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out_lines))
print("Done")
